...
const About: React.FC<AboutProps> = ({ content }) => {
  ...
  return (
    <section id="about" className="py-20 bg-gray-50">
      <div 
        ref={sectionRef}
        className={`container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
      >
        ...
        <div className="order-1 md:order-2">
            <div className="rounded-lg shadow-xl overflow-hidden aspect-w-1 aspect-h-1">
                <img src={siteImages.aboutUs} alt="A beautifully arranged, elegant event space planned by So Nice Event" className="w-full h-full object-cover" />
            </div>
        </div>
      </div>
    </section>
